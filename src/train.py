...
def train_parser():
    ...
    parser.add_argument('-amr_rnn_size', type=int, default=512)
    ...
    parser.add_argument('-dep_len', type=int,default=45,
                        help="size of dep linkage type ")
...
def eval(model,AmrDecoder,data,dicts,rel=False):
    ...
    for batchIdx in range(len(data)):
        ...
        if rel:
            ...
            rel_roles_prob,roots_log_soft_max = model((rel_batch_data,srcBatch,align_batch,fw_adjs,bw_adjs,fw_edgeid,bw_edgeid,fw_edgedep,bw_edgedep),rel=True)
...
def trainModel(model,AmrDecoder, trainData, validData, dicts, optim,best_f1 = 0 ,best_epoch = 0,always_print = False):
    ...
    def trainEpoch(epoch):
        ...
        for i in range(min(len(trainData),bs)):
            ...
            if opt.rel:
                rel_prob,roots = model((rel_batch,rel_index_batch,srcBatch, posteriors_likelihood_score[0],fw_adjs,bw_adjs,fw_edgeid,bw_edgeid,fw_edgedep,bw_edgedep),rel=True)
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        if opt.rel:
            cs=P_R_F1(*concept_scores[1].t_p_tp)
            rs = P_R_F1(*rel_scores[1].t_p_tp)
        ...
        varres["CS"].append(cs[2])
        varres["RS"].append(rs[2])
        varres["F"].append(p_r_f1[2])

        varres["CSP"].append(cs[0])
        varres["RSP"].append(rs[0])
        varres["FP"].append(p_r_f1[0])

        varres["CSR"].append(cs[1])
        varres["RSR"].append(rs[1])
        varres["FR"].append(p_r_f1[1])
...
def main():
    ...
    dicts = read_dicts()
    opt.dep_len = len(dicts["dep_dict"])
    ...
    training_data = DataIterator(trainingFilesPath, trainingAdjFilesPath,opt,dicts["rel_dict"],opt.dep_len)
    dev_data = DataIterator(devFilesPath, devAdjFilesPath, opt, dicts["rel_dict"],opt.dep_len, volatile=True)
...
if __name__ == "__main__":
    ...
    CUDA_CACHE_PATH = 'C:/Users/cyang26/cudacache'
