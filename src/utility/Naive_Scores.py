...
class ScoreHelper:
    ...
    def T_P_TP_Batch(self,hypos,golds,accumulate=True,second_filter_material =None):
        if self.second_filter:
            T,P,TP,fp,fn = T_P_TP_Batchyang(hypos,golds,self.f,self.second_filter,second_filter_material)
        else:
    #        assert self.name != "Unlabled SRL Triple",(hypos[-20],"STOP!",golds[-20])
            T,P,TP,fp,fn = T_P_TP_Batchyang(hypos,golds,self.f)
        if accumulate:
...
def T_P_TP_Batchyang(hypos,golds,filter=legal_concept,second_filter=None,second_filter_material_batch = None):
    TP,T,P = 0,0,0
    FP,FN = [],[]
    assert hypos, golds
