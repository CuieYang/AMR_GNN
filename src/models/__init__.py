...
#srl index is from original node to recategorized, combine with posterior of recategorized concepts
    ...
    def forward(self, input,rel=False):
        ...
        if len(input)==10 and rel:
            ...
            rel_batch,rel_index,srcBatch,posterior,fw_adjs,bw_adjs,fw_edgeid,bw_edgeid,fw_edgedep,bw_edgedep = input
            ...
            src_enc = self.rel_encoder(srcBatch,indexed_posterior,fw_adjs,bw_adjs,fw_edgeid,bw_edgeid,fw_edgedep,bw_edgedep)
     ...
     #       assert not np.isnan(np.sum(rel_prob[0].data.data.cpu().numpy())),("inside srl\n",rel_prob[0].data.data)
            return rel_prob
        if len(input)==9 and rel:
            # relation identification evaluation
            rel_batch,srcBatch,alginBatch,fw_adjs,bw_adjs,fw_edgeid,bw_edgeid,fw_edgedep,bw_edgedep = input   #
            src_enc = self.rel_encoder(srcBatch,alginBatch,fw_adjs,bw_adjs,fw_edgeid,bw_edgeid,fw_edgedep,bw_edgedep)
            root_enc =  self.root_encoder(srcBatch)
